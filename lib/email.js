...
async function sendPOEmail({ to, subject, text, attachment }) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    // Force STARTTLS on plaintext ports (587/2525): without requireTLS,
    // nodemailer falls back to cleartext when the server doesn't advertise
    // STARTTLS — SMTP credentials would travel in plaintext. Resend supports
    // STARTTLS on 587, so this is always safe in practice.
  ...
  try {
    await transporter.sendMail({
      from: process.env.FROM_EMAIL || 'stockyshift@stockyshift.com',
      to,
      subject,
      text,
      ...(attachment ? {
