...
/**
 * Generate a simple list/table PDF (products list, purchase orders list).
 * @param {Object} opts - { title, subtitle, columns: [{label,width,align}], rows: [string[]] }
 * @returns {Promise<Buffer>} PDF buffer
 */
function generateListPDF(opts) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 50 });
    const buffers = [];
    doc.on('data', chunk => buffers.push(chunk));
    ...
    try {
      // Compute x positions automatically from widths
      let cx = 50;
      ...
      // ─── Header ──────────────────────────────────────────────────────
      doc.fontSize(20).font('Helvetica-Bold').text(opts.title, { align: 'left' });
      if (opts.subtitle) {
        doc.moveDown(0.3);
        doc.fontSize(10).font('Helvetica').fillColor('#555').text(opts.subtitle);
        doc.fillColor('#000');
      }
      doc.moveDown(0.8);
      ...
      const headerY = doc.y;
      cols.forEach(col => {
        doc.text(String(col.label).toUpperCase(), col.x, headerY, { width: col.width, align: col.align || 'left' });
      });
      doc.moveDown(0.4);
      doc.moveTo(50, doc.y).lineTo(50 + cols.reduce((s, c) => s + c.width, 0), doc.y).stroke();
      doc.moveDown(0.3);
      ...
      for (const row of opts.rows || []) {
        const y = doc.y;
        cols.forEach((col, i) => {
          doc.text(trunc(row[i], 60), col.x, y, { width: col.width, align: col.align || 'left' });
        });
        doc.moveDown(0.7);
        if (doc.y > doc.page.height - 80) {
          ...
          const hdrY = doc.y;
          cols.forEach(col => {
            doc.text(String(col.label).toUpperCase(), col.x, hdrY, { width: col.width, align: col.align || 'left' });
          });
          doc.moveDown(0.4);
          doc.moveTo(50, doc.y).lineTo(50 + cols.reduce((s, c) => s + c.width, 0), doc.y).stroke();
          doc.moveDown(0.3);
...
module.exports = { generatePO, generateListPDF };
